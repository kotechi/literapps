<x-layouts::app :title="'Tambah buku'">
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('buku.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
                Kembali
            </a>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Tambah buku</h1>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-8">
            <form method="POST" action="{{ route('buku.store') }}">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label for="nama_buku" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Nama buku</label>
                        <input type="text" name="nama_buku" id="nama_buku" value="{{ old('nama_buku') }}"
                               class="block w-full px-4 py-3 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                               required>
                        @error('nama_buku')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <a href="{{ route('buku.index') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-semibold shadow-sm transition-all">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
